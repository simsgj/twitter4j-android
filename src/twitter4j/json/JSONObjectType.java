...
public final class JSONObjectType {
	...
	/**
	 * Determine the respective object type for a given JSONObject. This method
	 * inspects the object to figure out what type of object it represents. This
	 * is useful when processing JSON events of mixed type from a stream, in
	 * which case you may need to know what type of object to construct, or how
	 * to handle the event properly.
	 * 
		...
		if (!json.isNull("sender"))
			...
			try {
				...
				if ("favorite".equals(event))
					...
					else if ("list_destroyed".equals(event)) return USER_LIST_DESTROYED;
