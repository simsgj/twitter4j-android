...
public final class DataObjectFactory {
	...
	static {
		try {
			...
			tweetConstructor = (Constructor<Tweet>) Class.forName("twitter4j.internal.json.TweetJSONImpl")
					.getDeclaredConstructor(JSONObject.class);
			...
			rateLimitStatusConstructor = (Constructor<RateLimitStatus>) Class.forName(
					"twitter4j.internal.json.RateLimitStatusJSONImpl").getDeclaredConstructor(JSONObject.class);
			...
			relatedResultsConstructor = (Constructor<RelatedResults>) Class.forName(
					"twitter4j.internal.json.RelatedResultsJSONImpl").getDeclaredConstructor(JSONArray.class);
	...
	public static Object createObject(String rawJSON) throws TwitterException {
		try {
			...
			if (JSONObjectType.SENDER == jsonObjectType)
				...
				return registerJSONObject(statusDeletionNoticeConstructor.newInstance(json.getJSONObject("delete")
						.getJSONObject("status")), json);
				...
				return json;
			else if (JSONObjectType.SCRUB_GEO == jsonObjectType) // TODO:
																	// Perhaps
																	// there
																	// should be
																	// a
