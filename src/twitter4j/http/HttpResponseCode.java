...
public interface HttpResponseCode {
	...
	int NOT_ACCEPTABLE = 406;// Not Acceptable: Returned by the Search API when
								// an invalid format is specified in the
								// request.
	int TOO_LONG = 413;// Not Acceptable: Returned by the Search API when an
						// invalid format is specified in the request.
	/**
	 * @see <a
	 *      href="http://groups.google.com/group/twitter-api-announce/browse_thread/thread/3f3b0fd38deb9b0f?hl=en">Search
	 *      API: new HTTP response code 420 for rate limiting starting
	 *      1/18/2010</a>
	 */
	int ENHANCE_YOUR_CLAIM = 420;// Enhance Your Calm: Returned by the Search
									// and Trends API when you are being rate
									// limited. Not registered in RFC.
	int INTERNAL_SERVER_ERROR = 500;// Internal Server Error: Something is
									// broken. Please post to the group so the
									// Twitter team can investigate.
	int BAD_GATEWAY = 502;// Bad Gateway: Twitter is down or being upgraded.
	int SERVICE_UNAVAILABLE = 503;// Service Unavailable: The Twitter servers
									// are up, but overloaded with requests. Try
